...
#endif
#ifdef VENDOR_EDIT
//liuchao@RM.BSP.TP.BASIC, 2020/06/23, Add for notify TP display fps change

__attribute__((weak)) void syna_tcm_refresh_switch(int fps)
{
    return;
}
#endif /* VENDOR_EDIT */
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (cur_mode == 0 && dst_mode == 1) { /* 60 switch to 120 */
		...
		ret = 1;
	//liuchao@RM.BSP.TP.BASIC, 2020/06/23, Add for notify TP display fps change120hz
	syna_tcm_refresh_switch(60);
	return ret;
}
...
static int panel_doze_disable(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (hbm_temp) {
		pr_err("lcm: %s: hbm state %d, set panel hbm\n", __func__, hbm_temp);
