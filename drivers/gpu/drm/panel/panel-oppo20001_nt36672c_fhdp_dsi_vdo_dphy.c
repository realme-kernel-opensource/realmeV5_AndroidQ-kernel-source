...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0x45, 0xE0);
	lcm_dcs_write_seq_static(ctx, 0x46, 0xD9);
	lcm_dcs_write_seq_static(ctx, 0x47, 0xD2);
	lcm_dcs_write_seq_static(ctx, 0x48, 0xCB);
	lcm_dcs_write_seq_static(ctx, 0x49, 0xC4);
	lcm_dcs_write_seq_static(ctx, 0x4A, 0xBD);
	lcm_dcs_write_seq_static(ctx, 0x4B, 0xB6);
	lcm_dcs_write_seq_static(ctx, 0x4C, 0xB6);
	lcm_dcs_write_seq_static(ctx, 0x4D, 0xB0);
	lcm_dcs_write_seq_static(ctx, 0x4E, 0xAC);
	lcm_dcs_write_seq_static(ctx, 0x4F, 0xAA);
	lcm_dcs_write_seq_static(ctx, 0x50, 0xA7);
	lcm_dcs_write_seq_static(ctx, 0x51, 0xA4);
	lcm_dcs_write_seq_static(ctx, 0x52, 0xA1);
		...
		void *handle, unsigned int level)
{
	...
	pr_err("%s backlight = %d\n", __func__, level);
	bl_tb0[1] = 0x0007&(level >> 8);
	bl_tb0[2] = 0x00FF&(level);
	esd_brightness = level;
	if (!cb)
		return -1;
