...
void get_adsp_aee_buffer(unsigned long *vaddr, unsigned long *size)
{
    ...
    n = dump_adsp_shared_memory(buf, len, ADSP_A_LOGGER_MEM_ID);
...
void get_adsp_aee_ram_buffer(unsigned long *vaddr, unsigned long *size)
{
	u32 clk_cfg = 0, uart_cfg = 0, n = 0;
	u32 clk_mask = ADSP_CLK_UART_EN | ADSP_CLK_CORE_0_EN |
		       ADSP_CLK_CORE_1_EN;
...
EXPORT_SYMBOL(get_adsp_aee_ram_buffer);
				...
				char *buf, loff_t offset, size_t size)
{
	...
	if (offset == 0) /* only do ke ramdump once at start */
		get_adsp_aee_ram_buffer(&tmp[0], &tmp[1]);
