...
static int mt6360_chgdet_post_process(struct mt6360_pmu_chg_info *mpci)
{
	...
	case MT6360_CHG_TYPE_SDP:
		dev_info(mpci->dev, "%s: MT6360_CHG_TYPE_SDP \n", __func__);
		mpci->chg_type = STANDARD_HOST;
		break;
	case MT6360_CHG_TYPE_SDPNSTD:
		dev_info(mpci->dev, "%s: MT6360_CHG_TYPE_SDPNSTD \n", __func__);
		mpci->chg_type = NONSTANDARD_CHARGER;
		break;
	case MT6360_CHG_TYPE_CDP:
		dev_info(mpci->dev, "%s: MT6360_CHG_TYPE_CDP \n", __func__);
		mpci->chg_type = CHARGING_HOST;
		break;
	case MT6360_CHG_TYPE_DCP:
		dev_info(mpci->dev, "%s: MT6360_CHG_TYPE_DCP \n", __func__);
	...
	/* BC12 retry (SDP) */
	if (atomic_read(&mpci->bc12_retry_cnt) < 3 &&
		mpci->chg_type == STANDARD_HOST) {
		ret = mt6360_bc12_retry_wkard(mpci);
		dev_info(mpci->dev, "%s: charger type: SDP; Retry Det\n", __func__);
...
{
	pr_err("%s:  suspend 0:%d\n", __func__,suspend);
	if (!oppompci) {
		printk(KERN_ERR "%s NULL\n", __func__);
		return -1;
	}
	if (atomic_read(&oppompci->suspended) == 1){
		pr_err("%s:  1 suspend :%d\n", __func__,suspend);
		return -1;
	}
	pr_err("%s:  2 suspend :%d\n", __func__,suspend);
...
void mt6360_enable_hvdcp_detect(void)
{
	...
	if (true == is_mtksvooc_project) {
		if (mt6360_pmu_reg_write(oppompci->mpi, MT6360_PMU_DPDM_CTRL, 0x16) < 0) {
			dev_err(oppompci->dev, "%s: fail to write MT6360_PMU_DPDM_CTRL\n", __func__);
		}
		goto enable_hvdcp;
	}
...
#ifdef VENDOR_EDIT
//==========Shengguang.Zhu@RM.Camera.Driver, 20200622, add for flashlight ntc temperature=======
