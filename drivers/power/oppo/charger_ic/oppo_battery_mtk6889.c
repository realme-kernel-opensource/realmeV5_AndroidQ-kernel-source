...
void mt_set_chargerid_switch_val(int value)
{
	...
	pval.intval = 0;
	if(!value)
		chip->usb_psd.set_property(chip->usb_psy,POWER_SUPPLY_PROP_FLASH_SWITCH, &pval);*/
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return;
	}
...
/*static bool oppo_flashswitch_check_is_gpio(struct oppo_chg_chip *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return false;
	...
	if (gpio_is_valid(chip->normalchg_gpio.flashswitch_gpio)){
		pr_err("%s:ouyangbaili true\n", __func__);
		return true;
	}
...
static int oppo_flashswitch_gpio_init(struct oppo_chg_chip *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return -EINVAL;
	...
	chip->normalchg_gpio.pinctrl = devm_pinctrl_get(chip->dev);
	chip->normalchg_gpio.flashswitch_active = 
		pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, 
			"flashswitch_active");

	if (IS_ERR_OR_NULL(chip->normalchg_gpio.flashswitch_active)) {
		chg_err("get flashswitch_active fail\n");
		return -EINVAL;
	}
	chip->normalchg_gpio.flashswitch_sleep = 
			pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, 
				"flashswitch_sleep");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.flashswitch_sleep)) {
		chg_err("get flashswitch_sleep fail\n");
		return -EINVAL;
	}
...
void flash_switch_set(void)
{
	int i = 0;
	struct oppo_chg_chip *chip = g_oppo_chip;
	pr_err("%s:ouyangbaili 00\n", __func__);
	if (!chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return;
	}

	if (oppo_flashswitch_check_is_gpio(chip) == true) {
		pr_err("%s:ouyangbaili flash_switch:%d\n", __func__,chip->flash_switch);
		if (!IS_ERR_OR_NULL(chip->normalchg_gpio.flashswitch_active) && !IS_ERR_OR_NULL(chip->normalchg_gpio.flashswitch_sleep)) {
			if(chip->flash_switch){
				pr_err("%s:ouyangbaili flashswitch_active\n", __func__);
				pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.flashswitch_active);
			}else{
				pr_err("%s:ouyangbaili flashswitch_sleep\n", __func__);
				pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.flashswitch_sleep);

			}
		}
	}
}
//EXPORT_SYMBOL(flash_switch_set);
*/
//====================================================================//
#ifdef VENDOR_EDIT
...
/*
static int oppo_chg_flashswitch_parse_dt(struct oppo_chg_chip *chip)
{
	int rc = 0;
	struct device_node *node = NULL;
	...
	chip->normalchg_gpio.flashswitch_gpio =
			of_get_named_gpio(node, "qcom,flashswitch-gpio", 0);
	if (chip->normalchg_gpio.flashswitch_gpio <= 0) {
		chg_err("Couldn't read qcom,flashswitch-gpio rc = %d, qcom,flashswitch-gpio:%d\n",
				rc, chip->normalchg_gpio.flashswitch_gpio);
	} else {
		if (oppo_flashswitch_check_is_gpio(chip) == true) {
			rc = gpio_request(chip->normalchg_gpio.flashswitch_gpio, "flashswitch-gpio");
			if (rc) {
				chg_err("unable to request flashswitch-gpio:%d\n", chip->normalchg_gpio.flashswitch_gpio);
			} else {
				rc = oppo_flashswitch_gpio_init(chip);
				if (rc)
					chg_err("unable to init flashswitch-gpio:%d\n", chip->normalchg_gpio.flashswitch_gpio);
			}
		}
		chg_err("flashswitch-gpio:%d\n", chip->normalchg_gpio.flashswitch_gpio);
	}
...
static int oppo_chg_parse_custom_dt(struct oppo_chg_chip *chip)
{
	...
	/*rc = oppo_chg_flashswitch_parse_dt(chip);
	if (rc) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chg_flashswitch_parse_dt fail!\n", __func__);
		return -EINVAL;
	}*/
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
			if(((oppo_vooc_get_fastchg_started() == true) ||
				...
				//pr_err("%s: 001 FAST_CHG_TYPE :%d\n", __func__,val->intval);
			} else {
				val->intval = oppo_vooc_get_fast_chg_type();
				//pr_err("%s: 002 FAST_CHG_TYPE :%d\n", __func__,val->intval);
				if (val->intval == 0 && g_oppo_chip && g_oppo_chip->chg_ops->get_charger_subtype) {
					val->intval = g_oppo_chip->chg_ops->get_charger_subtype();
					//pr_err("%s: 003 FAST_CHG_TYPE :%d\n", __func__,val->intval);
				}
				if(val->intval == 0 && ((oppo_vooc_get_fastchg_started() == true) ||
				(oppo_vooc_get_fastchg_dummy_started() == true)) &&
				(oppo_vooc_get_fast_chg_type() == FASTCHG_CHARGER_TYPE_UNKOWN)){
					val->intval = CHARGER_SUBTYPE_FASTCHG_VOOC;
				//pr_err("%s: 004 FAST_CHG_TYPE :%d\n", __func__,val->intval);
				}
			}
			break;
...
int oppo_chg_set_qc_config(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->soc < 90
		...
		if ((chip->calling_on || chip->camera_on || chip->soc >= 90 || chip->batt_volt >= 4450
