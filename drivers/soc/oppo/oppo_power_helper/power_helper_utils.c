...
#define LOG_BUF_CACHE_SIZE   20
#define LOG_BUF_MAX_SIZE     640
#define LOG_BUF_SIZE     (LOG_BUF_MAX_SIZE-1)
...
static char* get_cache_buf(void)
{
	...
	for(i = 0; i < LOG_BUF_CACHE_SIZE; i++){
		if(log_helper_pri->buf[i][0] != '#')
			continue;
		else
			break;
	}
	i = ((i >= LOG_BUF_CACHE_SIZE) ? 0 : i);
	//pr_err("ymj get_cache_buf index: %d\n", i);
...
void power_helper_save_to_buf(char *fmt, ...)
{
	...
	va_start(argp, fmt);
	count += power_helper_get_utc_time(log_buf + count, LOG_BUF_SIZE - count);
	count += vsnprintf(log_buf + count, LOG_BUF_SIZE - count, fmt, argp);
	count = count > LOG_BUF_SIZE - 1 ? LOG_BUF_SIZE - 1 : count;
	va_end(argp);
...
static int __ref log_show_thread_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		for(i = 0; i < LOG_BUF_CACHE_SIZE; i++)
		{
			...
			if(log_helper_pri->buf[i][0] != '#'){
				//pr_err("ymj:log_show_thread_main data, index %d\n", i);
				kfifo_in(&log_helper_pri->fifo, buf_cache, strlen(buf_cache) + 2);
				log_helper_pri->buf[i][0] = '#';
			}
		}
